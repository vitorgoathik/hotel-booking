...
export async function searchDestination(query: string): Promise<DestResult | null> {
  const url = `${BASE}/searchDestination?query=${encodeURIComponent(query)}`;
  try {
    const res = await fetch(url, {
      headers: headers(),
      next: { revalidate: 86400 }, // dest IDs rarely change
    });
...
function parseLabel(label: string) {
  ...
  const roomsLeftMatch    = label.match(/only (\d+) left/i);
  const roomsLeft         = roomsLeftMatch ? parseInt(roomsLeftMatch[1], 10) : 10;
...
function mapHotel(
  ...
  const strike = price.strikethroughPrice
    ? (price.strikethroughPrice as Record<string, number>).value
    : undefined;

  const pricePerNight    = Math.round(gross / nights);
  const originalPerNight = strike ? Math.round(strike / nights) : undefined;
  ...
  const photoUrl  = photoUrls[0]?.replace("square500", "square1024") ?? undefined;
